...
def update_from_conversation(user_msg, response, session_id="default"):
    ...
    if name_m:
        set_state("user_name", name_m.group(1), confidence=0.9, source="user_statement")
    role_m = re.search(r"I(?:'m| am) (?:a |an )?(\w+)", user_msg, re.IGNORECASE)
    if role_m:
        set_state("user_type", role_m.group(1), confidence=0.8, source="user_statement")
...
class UserModel:
    ...
    def to_context_str(self):
        return get_world_model_context(self.session_id)
...
class ConversationState:
    ...
    def to_dict(self):
        return {"session_id": self.session_id, "turn_count": self.turn_count, "topic": self.topic}
