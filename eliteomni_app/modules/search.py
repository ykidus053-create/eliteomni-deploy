...
def tool_search(query: str, _raw: bool = False) -> str:
    ...
    try:
        ...
        if quality < 0.3 and not query.endswith('2026'):
            try:
                r2 = requests.get(f'{SEARXNG_URL}/search',
                    ...
                    headers={'User-Agent': 'Mozilla/5.0 (compatible; EliteOmni/17)'}, timeout=8)
        ...
        try:
            from modules.tools import tool_wolfram
...
def _openrouter_search_fallback(query: str) -> str:
    ...
    payload = _json.dumps({
        ...
        "messages": [{"role": "user", "content": f"Search the web and answer this query with current information. Be factual and cite sources where possible: {query}"}],
        "max_tokens": 1000,
        "temperature": 0.3,
    }).encode()
...
def tool_search_multi(user_msg: str) -> str:
    ...
    if any(t in user_msg.lower() for t in _academic_triggers):
        try:
            from modules.tools import tool_arxiv, tool_pubmed
            from concurrent.futures import ThreadPoolExecutor as _TPE, as_completed as _ac
            with _TPE(max_workers=2) as _ex:
                _futs = {
                    _ex.submit(tool_arxiv, user_msg[:150]): "arxiv",
                    _ex.submit(tool_pubmed, user_msg[:150]): "pubmed",
                }
                for _f in _ac(_futs):
                    _res = _f.result()
                    if _res:
...
def compress_history(history: list):
    ...
    if total_tokens <= CTX_TOKEN_BUDGET * 3 and len(history) <= CTX_WINDOW * 4:
