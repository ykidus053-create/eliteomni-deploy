...
STEP 5 REGRESS: Run EXEC() again to confirm fix and check no new bugs introduced
...
def build_system_prompt_cached(skill, memory, episodic, rlhf_note, ctx_summary="", complexity="medium"):
    from modules.validation import build_system_prompt
...
FORCE_SEARCH_PROMPT = """
<force_search>
MANDATORY SEARCH TRIGGERS — always call SEARCH() before answering if the query contains:
- "latest", "recent", "new", "today", "current", "now", "2025", "2026"
- "news", "update", "release", "announcement", "just", "this week", "this year"
- any AI model name (GPT, Claude, Gemini, Llama, Mistral, etc.)
- any company name in present tense (OpenAI, Google, Meta, Apple, etc.)
- any person's current role, status, or recent actions
NEVER answer these from training data — it will be outdated. SEARCH first, always.
</force_search>
"""
