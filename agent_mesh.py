...
class AgentMesh:
    ...
    def _hard_pipeline(self, msg, draft, skill, search_ctx):
        """Upgraded: Swarm Intelligence + Autonomous Agent."""
        t0 = time.time()
        
        # Upgraded: Swarm Delegation for massive multi-module tasks
        if skill == "coder" and len(msg.split()) > 15:
            try:
                from swarm_orchestrator import run_swarm
                swarm_result = run_swarm(msg, self._gen)
                if swarm_result and len(swarm_result) > 100:
                    draft = f"[PYTHON IMPL START]\n{swarm_result}\n[PYTHON IMPL END]"
                    print(f"[AgentMesh:hard] Swarm Intelligence completed, t={int((time.time()-t0)*1000)}ms")
            except Exception as e:
                print(f"[AgentMesh:hard] Swarm failed ({e}), falling back to standard pipeline.")
