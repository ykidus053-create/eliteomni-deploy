...
def should_use_voting(msg, skill, complexity):
    if complexity == "easy":
        return False
    if skill in VOTE_SKILLS and complexity == "hard":
        return True
...
def self_consistent_answer(generate_fn, msgs, n_samples=3, max_tokens=800):
    ...
    for i in range(n_samples):
        ...
        if i > 0:
            last = dict(temp[-1])
            last["content"] = last["content"] + " [Independent attempt " + str(i+1) + "]"
            temp[-1] = last
