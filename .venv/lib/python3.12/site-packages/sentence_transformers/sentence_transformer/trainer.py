...
class SentenceTransformerTrainer(BaseTrainer):
    ...
    Important attributes:
        ...
        - **model**: Always points to the :class:`~sentence_transformers.sentence_transformer.model.SentenceTransformer` model to be trained.
        ...
        - **place_model_on_device**: Whether or not to automatically place the model on the device - it will be set
          ...
          :attr:`~sentence_transformers.sentence_transformer.training_args.SentenceTransformerTrainingArguments.place_model_on_device` is overridden to return ``False``.
        - **is_in_train**: Whether or not a model is currently running :meth:`~sentence_transformers.sentence_transformer.trainer.SentenceTransformerTrainer.train` (e.g. when :meth:`~sentence_transformers.sentence_transformer.trainer.SentenceTransformerTrainer.evaluate` is called while
          in :meth:`~sentence_transformers.sentence_transformer.trainer.SentenceTransformerTrainer.train`)

    End-to-end examples:

        - `Training and Finetuning Embedding Models <https://huggingface.co/blog/train-sentence-transformers>`_: end-to-end training example with benchmarks against the base model and other embedding models.
        - `Training and Finetuning Multimodal Embedding & Reranker Models <https://huggingface.co/blog/train-multimodal-sentence-transformers>`_: multimodal training example (Visual Document Retrieval).
        - `Train 400x faster Static Embedding Models <https://huggingface.co/blog/static-embeddings>`_: training attention-free static embedding models for high-throughput CPU inference.
